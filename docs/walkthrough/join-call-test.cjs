...
(async () => {
	...
	const isMemberView =
		bodyText.includes("Join Now") || bodyText.includes("join");
	const hasModControls =
		bodyText.includes("Launch") || bodyText.includes("Moderator");
	const hasSignInPrompt = bodyText.includes("sign in to join");
	const hasLiveSession =
		bodyText.includes("Live now") || bodyText.includes("Live");
	...
	const joinBtn = await page
		.locator('button:has-text("Join Now"), button:has-text("join")')
		.first();
	if (await joinBtn.isVisible().catch(() => false)) {
		...
		const hasJitsiContainer = await page
			.locator(
				'[data-testid="jitsi-iframe-host"], iframe[src*="meet.clouddelnorte"]',
			)
			.count();
		const hasLoadingSpinner =
			(await page.locator("text=connecting").count()) +
			(await page.locator("text=Connecting").count()) +
			(await page.locator("text=loading").count());
		const hasError = await page
			.locator('[class*="error"], [class*="alert-error"]')
			.count();
		...
		await page.screenshot({
			path: "docs/walkthrough/join-call-test.png",
			fullPage: true,
		});
		...
		await page.screenshot({
			path: "docs/walkthrough/join-call-nobutton.png",
			fullPage: true,
		});
	}
	...
	// 5. Console messages
	console.log("\n5. Console messages:");
	const jitsiMsgs = consoleMessages.filter(
		(m) =>
			m.includes("jitsi") ||
			m.includes("token") ||
			m.includes("error") ||

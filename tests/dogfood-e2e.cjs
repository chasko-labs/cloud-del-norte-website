...
(async () => {
	...
	const modAuth = JSON.parse(
		execFileSync(
			...
			[
				...
				"--auth-parameters",
				`USERNAME=${CDN_ADMIN_USERNAME},PASSWORD=${CDN_ADMIN_PASSWORD}`,
				"--profile",
				"jitsi-video-hosting",
				"--region",
				"us-west-2",
	...
	const memAuth = JSON.parse(
		execFileSync(
			...
			[
				...
				"57eikmt418ea6vti2f6h0pl74r",
				"--auth-parameters",
				`USERNAME=${CDN_MEMBER_USERNAME},PASSWORD=${CDN_MEMBER_PASSWORD}`,
				"--profile",
				"jitsi-video-hosting",
				"--region",
				"us-west-2",
	...
	console.log(
		`   Launch: ${launch.ok ? "✓" : "✗"} — ${launch.status}, infra: ${launch.infrastructure_status}`,
	);
	...
	if (!status.live) {
		console.log("   ✗ FAIL — meeting not live");
		process.exit(1);
	}
	...
	await modPage.goto("https://quantum.clouddelnorte.org/dashboard/", {
	...
	await modPage.evaluate((tokens) => {
		...
		if (tokens.RefreshToken)
			sessionStorage.setItem("cdn.refreshToken", tokens.RefreshToken);
		const payload = JSON.parse(
			atob(tokens.IdToken.split(".")[1].replace(/-/g, "+").replace(/_/g, "/")),
		);
		if (payload.exp)
	...
	const hasModControls =
		modBody.includes("Launch") ||
		modBody.includes("Moderator") ||
		modBody.includes("End");
	const hasLiveStatus =
		modBody.includes("SESSION IN PROGRESS") ||
		modBody.includes("Live") ||
		modBody.includes("live");
	...
	await modPage.screenshot({
		path: "docs/walkthrough/dogfood-mod-dashboard.png",
		fullPage: true,
	});
	...
	if (await joinBtn.isVisible().catch(() => false)) {
		...
		const modConferenceIndicator = modJitsiFrame
			.locator(
				'[data-testid="toolbox"], [class*="toolbox"], [aria-label="Leave"], [aria-label="Hangup"]',
			)
			.first();
		...
		await modPage.screenshot({
			path: "docs/walkthrough/dogfood-mod-incall.png",
			fullPage: true,
		});
	...
	await memPage.goto("https://quantum.clouddelnorte.org/dashboard/", {
		waitUntil: "networkidle",
		timeout: 20000,
	});
	...
	const memHasLive =
		memBody.includes("SESSION IN PROGRESS") ||
		memBody.includes("Live") ||
		memBody.includes("live");
	const memHasJoin = memBody.includes("Join Now") || memBody.includes("Join");
	const memNoMod =
		!memBody.includes("Launch Session") && !memBody.includes("End Session");
	...
	if (await memJoin.isVisible().catch(() => false)) {
		...
		// Access the Jitsi iframe content via frameLocator
		const memJitsiFrame = memPage.frameLocator(
			'iframe[src*="meet.clouddelnorte"]',
		);

		// Wait for and click the pre-join "Join Meeting" button inside the iframe
		const memPrejoinBtn = memJitsiFrame
			.locator(
				'[data-testid="prejoin.joinMeeting"], button:has-text("Join meeting"), button:has-text("Join Meeting")',
			)
			.first();
		...
		// Wait for actual conference view (toolbar or hangup button appears)
		const memConferenceIndicator = memJitsiFrame
			.locator(
				'[data-testid="toolbox"], [class*="toolbox"], [aria-label="Leave"], [aria-label="Hangup"]',
			)
			.first();
		...
		console.log(
			"   Member conference toolbar visible: ✓ — member is IN the call",
		);

		await memPage.screenshot({
			path: "docs/walkthrough/dogfood-mem-incall.png",
			fullPage: true,
		});
	...
	console.log(
		`   Status: live=${finalStatus.live} ${!finalStatus.live ? "✓" : "✗"}`,
	);

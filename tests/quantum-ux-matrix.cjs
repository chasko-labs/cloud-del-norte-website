...
async function run() {
	...
	try {
		...
		const nameField = await page.$(
			"input[name='name'], input[placeholder*='name' i], [class*='awsui_input']",
		);
		await screenshot(page, "04-registration-form");
		record(
			"4. Registration form fields",
			!!nameField,
			nameField ? "input found" : "no input found",
		);
	} catch (e) {
	...
	try {
		...
		const inputs = await page.$$(
			"[class*='awsui_input'] input, input[type='text'], input[type='email']",
		);
	...
	try {
		...
		record(
			"7. Sessions/calendar/passkey",
			hasSessions,
			hasSessions ? "UI elements found" : "not found on page",
		);
	} catch (e) {
	...
	// 9. Locale toggle
	try {
		const localeBtn = await page.$(
			"[aria-label*='locale' i], [aria-label*='language' i], .quantum-pill:nth-child(2)",
		);
		if (localeBtn) {
	...
	try {
		...
		record(
			"10. Responsive (375px)",
			visible,
			"toolbar visible at mobile width",
		);
	} catch (e) {

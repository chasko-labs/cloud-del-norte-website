...
(async () => {
  ...
  const cardText = await page.$$eval('[class*="awsui_root"] *, [class*="awsui_content"] *', els => {
    return els.filter(el => el.offsetWidth > 0 && el.textContent.trim().length > 2 && el.children.length === 0)
      .map(el => {
  ...
  for (const t of cardText.slice(0, 15)) {
    ...
    const isMonospace = t.fontFamily.includes('JetBrains') || t.fontFamily.includes('monospace');
    const readability = [];
    ...
    if (readability.length > 0) {
      issues.push(`CARD TEXT: "${t.text}" — ${readability.join(', ')} | font: ${t.fontFamily.substring(0, 30)}`);
      console.log(`  ✗ "${t.text}" | ${t.fontSize} ${t.fontWeight}w lh:${lh.toFixed(2)} | ${readability.join(', ')}`);
    }
  }
  ...
  for (const t of smallTargets) {
    issues.push(`SMALL TARGET: <${t.tag}> "${t.text}" — ${t.width}×${t.height}px`);
    console.log(`  ✗ <${t.tag}> "${t.text}" — ${t.width}×${t.height}px`);
  }

  // 3. Mobile check
  await ctx.close();
  ctx = await browser.newContext({ viewport: { width: 375, height: 812 }, colorScheme: 'dark' });
  page = await ctx.newPage();
  await page.goto('https://quantum.clouddelnorte.org/', { waitUntil: 'networkidle', timeout: 20000 });
  await page.waitForTimeout(2000);

  const mobileSmallTargets = await page.$$eval('button, a, [role="button"]', els => {
    return els.filter(el => {
      const rect = el.getBoundingClientRect();
      return rect.width > 0 && rect.height > 0 && (rect.width < 44 || rect.height < 44);
    }).map(el => {
      const rect = el.getBoundingClientRect();
      return { text: (el.textContent || '').trim().substring(0, 30), width: Math.round(rect.width), height: Math.round(rect.height) };
    }).slice(0, 10);
  });

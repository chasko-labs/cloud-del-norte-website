...
(async () => {
	...
	for (const mode of ["dark", "light"]) {
		...
		const elements = await page.$$eval(
			"h1, h2, h3, p, span, a, button, div, header",
			(els) => {
				return els
					.filter((el) => {
						const text = (el.textContent || "").trim();
						if (!text || text.length < 2 || el.children.length > 3)
							return false;
		...
		for (const el of elements) {
			const probs = [];
			if (el.contrast && parseFloat(el.contrast) < 4.5 && el.fontSize < 18)
				probs.push(`LOW CONTRAST ${el.contrast}:1`);
			if (el.contrast && parseFloat(el.contrast) < 3.0 && el.fontSize >= 18)
				probs.push(`LOW CONTRAST (large) ${el.contrast}:1`);
			if (el.fontSize < 14) probs.push(`TOO SMALL ${el.fontSize}px`);
			if (parseFloat(el.lineHeight) < 1.3)
				probs.push(`TIGHT LINE-HEIGHT ${el.lineHeight}`);
			if (el.fontWeight < 400 && el.fontSize < 18)
				probs.push(`THIN ${el.fontWeight}w`);
			if (el.opacity < 0.9) probs.push(`LOW OPACITY ${el.opacity}`);

			if (probs.length > 0) {
				issues.push({ mode, ...el, problems: probs });
				console.log(
					`  ✗ "${el.text}" | ${el.fontSize}px ${el.fontWeight}w | contrast:${el.contrast || "?"} | ${probs.join(", ")}`,
				);
			}
		}
		await ctx.close();

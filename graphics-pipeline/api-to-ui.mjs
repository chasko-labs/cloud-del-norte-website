...
const api = JSON.parse(readFileSync(join(dir, "cdn-composite.api.json"), "utf8"));

// link-typed inputs (in order) and widget inputs (in order) + output types per node type
const SPEC = {
	CheckpointLoaderSimple: { links: [], widgets: ["ckpt_name"], outs: ["MODEL", "CLIP", "VAE"] },
	CLIPTextEncode: { links: ["clip"], widgets: ["text"], outs: ["CONDITIONING"] },
	EmptyLatentImage: { links: [], widgets: ["width", "height", "batch_size"], outs: ["LATENT"] },
	KSampler: {
		links: ["model", "positive", "negative", "latent_image"],
		widgets: ["seed", "__ctrl", "steps", "cfg", "sampler_name", "scheduler", "denoise"],
		outs: ["LATENT"],
	},
	VAEDecode: { links: ["samples", "vae"], widgets: [], outs: ["IMAGE"] },
	LoadImage: { links: [], widgets: ["image", "__upload"], outs: ["IMAGE", "MASK"] },
	ImageBlur: { links: ["image"], widgets: ["blur_radius", "sigma"], outs: ["IMAGE"] },
	ImageScale: { links: ["image"], widgets: ["upscale_method", "width", "height", "crop"], outs: ["IMAGE"] },
	ImageBlend: { links: ["image1", "image2"], widgets: ["blend_factor", "blend_mode"], outs: ["IMAGE"] },
	ImageCompositeMasked: { links: ["destination", "source", "mask"], widgets: ["x", "y", "resize_source"], outs: ["IMAGE"] },
...
for (const id of ids) {
	...
	for (const name of spec.links) {
		...
		inputs.push({ name, type: outType(api, ref[0], ref[1]), link: null, _ref: ref });
	}
	...
	nodes.push({
		...
		mode: 0,
		inputs,
		outputs: spec.outs.map((t, i) => ({ name: t.toLowerCase(), type: t, links: [], slot_index: i })),
...
for (const n of nodes) {
	for (const inp of n.inputs) {
		...
		links.push([linkId, Number(srcId), srcSlot, n.id, n.inputs.indexOf(inp), inp.type]);
		delete inp._ref;
	}
}
...
writeFileSync(join(dir, "cdn-composite.ui.json"), `${JSON.stringify(ui, null, 2)}\n`);
console.log(`wrote cdn-composite.ui.json: ${nodes.length} nodes, ${links.length} links`);

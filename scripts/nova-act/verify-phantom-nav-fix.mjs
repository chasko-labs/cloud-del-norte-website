...
const require = createRequire('/home/bryanchasko/.nvm/versions/node/v24.14.0/lib/node_modules/playwright/');
...
async function run() {
  ...
  try {
    ...
    }, { timeout: 20000 }).catch(() => console.log('[warn] Home page React root slow'));
    ...
    const createMeetingFound = navItems.some(d => d.text.toLowerCase().includes('create meeting'));
    evidence.create_meeting_found = createMeetingFound;

    if (!createMeetingFound) {
      results.create_meeting_hidden = 'PASS';
      console.log('[CHECK] PASS: "create meeting" link NOT visible for non-moderator');
    } else {
      console.log('[CHECK] FAIL: "create meeting" link IS visible for non-moderator');
    }
    ...
    const response = await page.goto(ADMIN_URL, { waitUntil: 'networkidle', timeout: 30000 });
    console.log(`[admin] Response status: ${response?.status()}, URL after goto: ${page.url()}`);

    // Wait for React to mount and render content inside #root
    await page.waitForFunction(() => {
      const root = document.getElementById('root');
      return root && root.innerHTML.trim().length > 50;
    }, { timeout: 30000 }).catch(() => console.log('[warn] React root did not populate within 30s'));
    await page.waitForTimeout(3000);

    // Check if we got redirected (requireAuth might redirect to login)
    const finalUrl = page.url();
    console.log(`[admin] Final URL: ${finalUrl}`);
    console.log(`[admin] Console errors: ${JSON.stringify(consoleMessages.slice(0, 5))}`);
    ...
    const denialKeywords = [
      "don't have access", "moderators only", "moderator access",
      "no tienes acceso", "solo para moderadores",
      "access denied", "not authorized"
    ];
    const hasDenial = denialKeywords.some(kw => bodyText.toLowerCase().includes(kw));
    ...
    if ((hasDenial || hasAlert) && !isBlank) {
      ...
      console.log(`[CHECK] PASS: access-denied card visible (denial_text=${hasDenial}, alert=${hasAlert})`);
    } else {
      console.log(`[CHECK] FAIL: access-denied card NOT visible (denial_text=${hasDenial}, alert=${hasAlert}, blank=${isBlank})`);
    }

    // Capture DOM snippet for evidence
    const adminHtml = await page.innerHTML('main, [class*="awsui_content"], body').catch(() => '');
...
writeFileSync(evidencePath, JSON.stringify({ results, evidence, timestamp: TS, user: EMAIL }, null, 2));
...
console.log(`Screenshots: ${evidence.screenshot_nav || 'N/A'}, ${evidence.screenshot_admin || 'N/A'}`);

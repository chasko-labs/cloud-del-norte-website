...
function selectMeetup(events) {
	...
	const withDates = events.filter(
		(e) => e.dtstart instanceof Date && !Number.isNaN(e.dtstart.getTime()),
	);
...
function shapeForOutput(meetup) {
	...
	// Description excerpt cap — wave 43a lowered 2000 → 320. The
	// renderDescriptionWithLinks helper now strips markdown noise before
	// render, so we can trim earlier; 320 keeps a useful teaser while
	// cutting the bulk of the raw markdown that cluttered the card.
	const description = meetup.description
		? String(meetup.description).replace(/\s+/g, " ").trim().slice(0, 320)
		: "";
	return {
...
try {
	...
	console.log(
		`[fetch-next-meetup] parsed ${events.length} events from ical feed`,
	);
	const selected = selectMeetup(events);
	payload = shapeForOutput(selected);
	if (payload) {
		console.log(
			`[fetch-next-meetup] selected ${payload.status} event: ${payload.title} (${payload.dtstart})`,
		);
	} else {
		console.log(

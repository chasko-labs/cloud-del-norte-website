...
async function measure(viewport) {
	...
	const cards = await page.evaluate(() => {
		const results = [];
		document
			.querySelectorAll(".feed-carousel__frame, .feed-twitch__frame")
			.forEach((el) => {
				const rect = el.getBoundingClientRect();
				const card = el.closest(".cdn-card, .feed-grid__cell");
				const name =
					card
						?.querySelector("[class*='awsui_header']")
						?.textContent?.trim()
						?.slice(0, 40) || "unknown";
				results.push({
					name,
...
for (const [label, vp] of [
	...
	try {
		...
		const benchmark =
			cards.find((c) => c.name.toLowerCase().includes("youtube")) ?? cards[0];
		const benchmarkHeight = benchmark?.computed_height ?? 315;
		const oversized = cards.filter(
			(c) => c.computed_height > benchmarkHeight * 1.2,
		);
		console.log(
			JSON.stringify(
				{
					cards,
					benchmark_height: benchmarkHeight,
					oversized: oversized.map((c) => c.name),
				},
				null,
				2,
			),

...
if (!FORCE && existsSync(OUT_PATH)) {
	...
	if (age < MAX_AGE_MS) {
		console.log(
			`releases.generated.json is fresh (${Math.round(age / 60000)}m old). Skipping fetch. Use --force to override.`,
		);
		process.exit(0);
	}
}
...
if (!GITHUB_TOKEN) {
	console.warn(
		"⚠️  GITHUB_TOKEN not set — using unauthenticated GitHub API (60 req/hr limit)",
	);
}
...
async function fetchAWSWhatsNew() {
	...
	const bedrockEntry =
		extractItems((i) => /bedrock|sagemaker/i.test(i))[0] ?? null;
	const iotEntry = extractItems((i) => /iot/i.test(i))[0] ?? null;
	const aiEntry =
		extractItems((i) =>
			/machine learning|ai|artificial intelligence/i.test(i),
		)[0] ?? null;

	console.log(
		`[fetch-releases] AWS RSS matches — lambda:${lambdaEntry ? 1 : 0} s3:${s3Entry ? 1 : 0} bedrock:${bedrockEntry ? 1 : 0} iot:${iotEntry ? 1 : 0} ai:${aiEntry ? 1 : 0}`,
	);
...
for (const manualEntry of manual) {
	if (results.has(manualEntry.id)) {
		results.set(manualEntry.id, {
			...results.get(manualEntry.id),
			...manualEntry,
		});
	}
}
...
for (const entry of results.values()) {
	if (entry.lastManualUpdate) {
		...
		if (age > STALE_THRESHOLD_MS) {
			console.warn(
				`⛔ STALE: ${entry.name} manual data is ${Math.round(age / 86400000)} days old`,
			);
		}
	}
}

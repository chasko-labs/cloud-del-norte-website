...
function parseIcal(text) {
  ...
  const now = new Date();
  const future = events.filter((e) => new Date(e.dtstart) >= now).sort((a, b) => a.dtstart.localeCompare(b.dtstart));
...
try {
  const res = await fetch(ICAL_URL, { headers: { "User-Agent": "cdn-website-ci/1.0" } });
  if (!res.ok) throw new Error(`HTTP ${res.status}`);
  const event = parseIcal(await res.text());
  if (!event) { console.warn("no events found"); process.exit(0); }
  mkdirSync(dirname(OUT), { recursive: true });
  writeFileSync(OUT, JSON.stringify({ ...event, fetchedAt: new Date().toISOString() }, null, 2));
  console.log(`✓ ${OUT}`);
} catch (err) {

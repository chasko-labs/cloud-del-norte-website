...
import { forgotPassword, confirmForgotPassword, AuthError, assertNonEmpty } from '../../../lib/cognito';
...
function ForgotPasswordForm() {
  ...
  async function handleReset(e: React.FormEvent) {
    ...
    try {
      ...
      setPasswordError(t('auth.forgotPassword.newPasswordLabel') + ' is required');
      valid = false;
    }
    if (!valid) return;
    setLoading(true);
  ...
  if (phase === 'done') {
    return (
      <Container>
        <SpaceBetween size="m">
          <Alert type="success">Password updated — you can now sign in with your new password.</Alert>
          <Box textAlign="center">
            <Link href="/login/index.html">{t('auth.forgotPassword.backToSignIn')}</Link>
          </Box>
        </SpaceBetween>
      </Container>
    );
    ...
    return (
      <Container>
        ...
        >
          ...
          >
            <SpaceBetween size="m">
              ...
              <FormField label={t('auth.forgotPassword.codeLabel')} errorText={codeError || undefined}>
                <Input
                  value={code}
        ...
        <Box margin={{ top: 'm' }} textAlign="center">
          <Link onFollow={() => setPhase('request')}>Back — request new code</Link>
        </Box>
      </Container>
    );
  }
  ...
  return (
    <Container>
      ...
      >
        ...
        >
          <SpaceBetween size="m">
            <FormField label={t('auth.forgotPassword.emailLabel')} errorText={emailError || undefined}>
              <Input
                type="email"
                value={email}

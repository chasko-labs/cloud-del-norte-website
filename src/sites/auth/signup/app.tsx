...
function SignupWizard() {
  ...
  function validateStep1(): boolean {
    ...
    if (password && confirmPassword !== password) errs.confirmPassword = t('auth.signup.passwordMismatch');
    setStep1Errors(errs);
  ...
  async function handleNavigate({ detail }: { detail: { requestedStepIndex: number } }) {
    ...
    if (requestedStepIndex > activeStepIndex) {
      ...
      if (activeStepIndex === 2 && !signUpCalled) {
        ...
        setFormError('');
        try {
          await signUp({ email, password, memberType, location, topics, background });
    ...
    try {
      ...
      if (err instanceof AuthError) {
        if (err.code === 'CodeMismatchException') {
          setStep4Errors({ code: 'Incorrect code — check your email and try again' });
  ...
  const steps = [
    {
      ...
      content: (
        <SpaceBetween size="m">
          <FormField label={t('auth.signup.emailLabel')} errorText={step1Errors.email ?? undefined}>
            <Input
              type="email"
              value={email}
          ...
          >
            <SpaceBetween size="xs">
              ...
              />
              <Link onFollow={() => setShowPassword(p => !p)}>{showPassword ? 'Hide password' : 'Show password'}</Link>
            </SpaceBetween>
          </FormField>
          <FormField label={t('auth.signup.confirmPasswordLabel')} errorText={step1Errors.confirmPassword ?? undefined}>
            <Input
              type={showPassword ? 'text' : 'password'}
          ...
          <Box textAlign="center">
            <Link href="/login/index.html">{t('auth.signup.alreadyHaveAccount')}</Link>
          </Box>
        </SpaceBetween>
      ),
    },
    ...
    },
    {
      ...
      content: (
        <SpaceBetween size="m">
          ...
          <FormField label={t('auth.signup.topicsLabel')} constraintText={`${topics.length} / ${FIELD_LIMITS.topics}`}>
            <Input
              value={topics}
    ...
    },
    {
      ...
      content: (
        <SpaceBetween size="m">
          ...
          <FormField label={t('auth.signup.verifyCodeLabel')} errorText={step4Errors.code ?? undefined}>
            <Input
              value={code}
          ...
          >
            {cooldown > 0
              ? t('auth.signup.resendCooldown').replace('{{seconds}}', String(cooldown))

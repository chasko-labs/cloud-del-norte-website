...
export default function YoutubeCarousel() {
  ...
  const prev = () => setCurrent(c => (c - 1 + VIDEO_IDS.length) % VIDEO_IDS.length);
    ...
    <Container header={<Header variant="h2">on YouTube</Header>}>
      <div className="feed-carousel">
        ...
        <div className="feed-carousel__controls">
          <button className="feed-carousel__btn" onClick={prev} aria-label="Previous video">
            &#8592;
          </button>
          <span className="feed-carousel__counter">
            {current + 1} / {VIDEO_IDS.length}
          </span>
          <button className="feed-carousel__btn" onClick={next} aria-label="Next video">
            &#8594;
          </button>
        </div>
      </div>

...
describe("applyManualOverrides", () => {
	...
	it("manual always beats a fetched partial regardless of application order", () => {
		...
		const afterFetch = mergePartials(
			[baseTech],
			[{ id: "python", name: "Python Fetched" }],
		);
		const afterManual = applyManualOverrides(afterFetch, [
			{ id: "python", name: "Python Manual" },
		]);
		expect(afterManual.find((t) => t.id === "python")?.name).toBe(
			"Python Manual",
		);

		// Apply manual first, then fetch partial — manual should still win (re-apply manual)
		const withFetchOnly = mergePartials(
			[baseTech],
			[{ id: "python", name: "Python Fetched" }],
		);
		const reapplied = applyManualOverrides(withFetchOnly, [
			{ id: "python", name: "Python Manual" },
		]);
		expect(reapplied.find((t) => t.id === "python")?.name).toBe(
			"Python Manual",
		);
	});
});

...
import { downloadICS, generateICS, generateICSForTech } from "../ical";
	...
	describe("generateICS", () => {
		...
		it("includes events from all techs when given multiple", () => {
			...
			const tech2 = makeTech({
				id: "beta",
				name: "Beta",
				projectedNextVersion: projected("2025-10-01"),
			});
		...
		it("uses CRLF line endings throughout (RFC 5545)", () => {
			...
			const linesWithBareNewline = output
				.split("\n")
		...
		it("includes DESCRIPTION from basedOn field", () => {
			const tech = makeTech({
				projectedNextVersion: projected("2025-09-15", "annual release pattern"),
			});
			const output = generateICS([tech]);

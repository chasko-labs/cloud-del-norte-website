import type { ReleaseEntry, ProjectedEntry, Confidence } from '../types';
 ...
 */
export function confidenceFromN(n: number, highThreshold: number = N_ANY): Confidence {
...
export function projectNextVersion(
  ...
  const mostRecent = releases.reduce((a, b) => new Date(a.date) > new Date(b.date) ? a : b);
  const projected = new Date(new Date(mostRecent.date).getTime() + avgDays * 86400000);
...
export function projectNextLTS(
  ...
  const mostRecent = ltsReleases.reduce((a, b) => new Date(a.date) > new Date(b.date) ? a : b);
  const projected = new Date(new Date(mostRecent.date).getTime() + avgDays * 86400000);

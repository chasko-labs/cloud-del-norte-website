...
export default function SpeakerProposalsAdminTable() {
	...
	const load = useCallback(async (status: Proposal["status"]) => {
		setLoading(true);
		setError(null);
		try {
			...
			<Table
				...
				columnDefinitions={[
					...
					},
					{
						...
						cell: (p) =>
							t(
								`admin.proposals.status.${p.format}` as Parameters<typeof t>[0],
							) || p.format,
					},
					{
						id: "earliestDate",
					...
					},
					{
						...
						cell: (p) => (
							<Box
								color={
									STATUS_TYPE[p.status] === "success"
										? "text-status-success"
										: STATUS_TYPE[p.status] === "error"
											? "text-status-error"
											: "text-status-info"
								}
							>
								{t(
									`admin.proposals.status.${p.status}` as Parameters<
										typeof t
									>[0],
								) || p.status}
							</Box>
					...
					},
					{
						...
						cell: (p) => (
							<SpaceBetween direction="horizontal" size="xs">
								...
								<Button
									onClick={() => {
										window.location.href = `/create-meeting/index.html?proposalId=${p.id}&topic=${encodeURIComponent(p.topic)}&speakers=${encodeURIComponent(p.name)}&earliestDate=${p.earliestDate}`;
									}}
								>
